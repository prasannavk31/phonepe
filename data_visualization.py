...
try:
    conn = msql.connect(host='localhost',database='assignment_guvi', user='root', password='')
...
state_lat_lon = pd.read_csv('/Users/prasanna/vs_code/twitter/state_lat_lon.csv')
...
if menu == 'Geo Map':
    ...
    state_lat_lon.rename(columns = {'State.Name':'State'}, inplace = True)
    #lat_lon_df = pd.merge(state_lat_lon, yrQtrTrans_filter,left_on='State.Name',right_on='State',how='left')
    ...
    india_states = gdf.rename(columns={"NAME_1": "State"}).__geo_interface__
    ...
    fig = px.choropleth(
          pd.json_normalize(india_states["features"])["properties.State"],
          locations="properties.State",
